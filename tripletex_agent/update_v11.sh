#!/usr/bin/env bash
set -euo pipefail
echo "=== v11: Fix travel expense, payment endpoint, customer name, person name, org number ==="
...
"""Orchestrator v11 - fix travel expense, payment endpoint, customer name.

New in v11:
- Travel expense: try 3 payload variants (no date, date, startDate/endDate)
- Payment: try 4 endpoints (/:payment, /:createPayment, /payment, /bank/payment)
- Customer name: 'com' terminator, payment-specific pattern
- Person name: extract from 'Name (email)' pattern
- Org number: 'número de organização' pattern

Carried from v10:
- register_payment handler, process_salary, payment/salary priority detection
"""
import json
import sys
import time
...
echo "=== Deploying v11 to Cloud Run ==="
cd ~/tripletex_agent
