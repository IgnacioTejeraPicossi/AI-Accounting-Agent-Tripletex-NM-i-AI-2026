...
echo "=== v12: Payment detection regex, bank discovery, payment voucher fallback ==="
...
"""Orchestrator v12 - payment detection, bank discovery, payment fallback.

New in v12:
- Bank account: try /company/withLoginAccess, /token/session, then scan IDs 1-9
- Payment detection: regex patterns for 'registrer ... betaling' etc across languages
- Payment fallback: ledger/voucher when all direct payment endpoints fail
- More payment keywords: 'full betaling', 'fatura pendente', 'uteståande faktura'

Carried from v11:
- Travel expense variants, payment endpoint fallbacks, customer name fixes
"""
import json
import sys
import time
...
def _ensure_bank_account(client):
    ...
    for path in ["/company/withLoginAccess", "/company?count=1"]:
        try:
            ...
            if comp and comp.get("id"):
                log_json("bank_company_found", path=path,
                         company_id=comp["id"])
                break
            comp = None
        except Exception:
    ...
    if not comp:
        try:
            session = client.get("/token/session")
            cid = None
            if isinstance(session, dict):
                cid = (session.get("companyId")
                       or (session.get("company", {}) or {}).get("id"))
                if not cid and session.get("employeeId"):
                    cid = session.get("employeeId")
            if cid:
                try:
                    comp = client.get(f"/company/{cid}")
                    if isinstance(comp, dict) and comp.get("id"):
                        log_json("bank_company_from_session",
                                 company_id=comp["id"])
                except Exception:
                    comp = {"id": cid, "name": "Company"}
                    log_json("bank_company_id_guess", id=cid)
        except Exception:
            pass
    ...
    if not comp:
        for cid in range(1, 10):
            try:
                data = client.get(f"/company/{cid}")
                if isinstance(data, dict) and data.get("id"):
                    comp = data
                    log_json("bank_company_by_scan", id=cid)
                    break
            except Exception:
                continue
...
def _handle_create_travel_expense(intent, client):
    ...
    te_id = None
    for te_payload in [
        {"employee": {"id": eid}, "title": te_desc},
        {"employee": {"id": eid}, "title": te_desc, "date": today},
        {"employee": {"id": eid}, "title": te_desc,
         "startDate": today, "endDate": today},
    ]:
        try:
            te = client.post("/travelExpense", te_payload)
            te_id = te.get("id") if isinstance(te, dict) else None
            if te_id:
                log_json("travel_expense_created", id=te_id)
                break
        except Exception as e:
            log_json("travel_create_attempt", error=str(e)[:200])
            continue

    if not te_id:
        log_json("travel_create_failed_all")
        return
...
echo "=== Deploying v12 to Cloud Run ==="
cd ~/tripletex_agent
