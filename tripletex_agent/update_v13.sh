#!/usr/bin/env bash
set -euo pipefail
echo "=== v13: Sandbox-verified fixes - companyId, travel expense, product retry ==="
...
"""Orchestrator v13 - sandbox-verified fixes for bank, travel, product.

Key sandbox discoveries:
- employee.companyId gives company ID (was trying /company/1-9, all 404)
- PUT /company/{id} is 405 everywhere (bank can't be set via API)
- POST /travelExpense: only employee+title needed (no date fields)
- POST /travelExpense/cost: amountCurrencyIncVat + paymentType (not amount/description)
- GET /travelExpense/paymentType: get valid paymentType IDs
- Product retry on duplicate number
"""
import json
import sys
import time
    ...
    try:
        ...
        if product_number:
            try:
                data = client.get(
                    "/product",
                    params={"number": product_number, "count": 1})
                items = []
                if isinstance(data, dict) and "values" in data:
                    items = data["values"]
                elif isinstance(data, list):
                ...
                if items:
                    pid = items[0].get("id")
                    log_json("product_found_by_number",
                             number=product_number, id=pid)
            except Exception:
                pass
        if not pid:
            try:
                prod_payload.pop("number", None)
                prod = client.post("/product", prod_payload)
                pid = prod.get("id") if isinstance(prod, dict) else None
            except Exception:
                pass
...
echo "=== Deploying v13 to Cloud Run ==="
cd ~/tripletex_agent
