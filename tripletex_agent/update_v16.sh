...
echo "=== v16: order vs payment, PDF merge, book_expense_receipt, salary email ==="
...
def detect_entity(text):
    ...
    if re.search(r"kvittering|togbillett|receipt", lower) and re.search(
        ...
        if re.search(r"bokf|book|posting|registrer|utgift", lower, re.IGNORECASE):
...
"""Orchestrator v16 - order vs payment, receipt PDF, salary email, expense voucher.
