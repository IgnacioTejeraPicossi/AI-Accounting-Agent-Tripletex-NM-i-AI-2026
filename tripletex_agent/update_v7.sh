...
echo "=== v7: Supplier, credit note, product details, employee dept, bank account ==="
...
"""Orchestrator v7 - comprehensive fix for all task types.

Key changes:
- Supplier entity + handler (isSupplier on /customer)
- Credit note handler (find invoice, create credit note)
- Product creation with number, price, VAT
- Employee creation with department (required field)
- Project manager: find existing employee before creating
- Invoice: bank account setup before creating
"""
import json
import sys
import time
...
def _ensure_bank_account(client):
    """Attempt to set up a bank account for invoicing."""
    try:
        data = client.get("/company", params={"count": 1})
        comp = None
        ...
        client.put(f"/company/{comp_id}", {
            "name": comp.get("name", "Company"),
            "bankAccountNumber": "15032457284",
        })
        log_json("bank_account_set", company_id=comp_id)
    except Exception as e:
        log_json("bank_setup_error", error=str(e)[:200])
...
echo "=== Deploying v7 to Cloud Run ==="
cd ~/tripletex_agent
