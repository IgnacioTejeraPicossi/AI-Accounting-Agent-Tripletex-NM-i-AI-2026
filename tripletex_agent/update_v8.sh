...
echo "=== v8: Travel expense, order, supplier invoice, bank fix, supplier name ==="
...
def extract_invoice_lines(text):
    ...
    # "ProductName (ProductNumber) à Price NOK" without VAT
    multi2 = re.findall(
        r'([\w]+(?:\s+[\w]+)*?)\s*\((\d{3,})\)\s*'
        r'(?:til|at|\u00e0|a|zu|por|for)\s+'
        r'(\d[\d\s]*)\s*(?:kr|NOK|EUR|USD)',
        text, re.IGNORECASE | re.UNICODE)
    if multi2:
        for name, number, price_str in multi2:
            clean_name = re.sub(
                r'^(?:og|and|y|e|et|und|,)\s+', '', name.strip(),
                flags=re.IGNORECASE).strip()
            price = float(price_str.replace(" ", "").replace(",", "."))
...
"""Orchestrator v8 - travel expense, order, supplier invoice + bank fix.

New in v8:
- Bank account: try GET /company/1 then /company/2 (proxy rejects /company)
- Travel expense handler (create report + costs + per diem)
- Order handler (create order -> invoice -> payment)
- Supplier invoice handler (try /supplierInvoice, /purchaseOrder, voucher)
- Supplier name cleanup (no more 'Registrer leverandøren' prefix)

Carried from v7:
- Supplier entity + handler, credit note, product details, dept for employees
"""
import json
import sys
import time
...
echo "=== Deploying v8 to Cloud Run ==="
cd ~/tripletex_agent
