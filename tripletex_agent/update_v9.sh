#!/usr/bin/env bash
set -euo pipefail
echo "=== v9: Project startDate, customer name/org/address, project name fix ==="
...
"""Orchestrator v9 - project startDate fix, customer name/org/address.

New in v9:
- Project: always include startDate (today if not specified) - was 422 mandatory
- Customer: org_number, address, phone in payload
- Customer name: use extract_customer_name_for_invoice (not suffix-greedy)
- Project name: prefer quoted name after 'project' keyword
- Company suffixes: added Lda, SARL, Ltda, Srl

Carried from v8:
- Bank account, travel expense, order, supplier invoice, supplier name
"""
import json
import sys
import time
...
echo "=== Deploying v9 to Cloud Run ==="
cd ~/tripletex_agent
